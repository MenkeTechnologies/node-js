//! Offline generator for `docs/reference.html` — the keyword / global / method
...
//! ```
//!
//! Source of truth: the corpus in `nodejs::lsp` (`corpus()`), the exact
//! `(name, chapter, doc, example)` table. The static page therefore never drifts
//! from the runtime — a name is documented here only if the crate actually
//! recognizes it in `parser.rs` (keywords) or `builtins.rs` (globals / methods).
//!
//! Chapters are the corpus's own keyword/global/namespace/type grouping
//! (`entry.1`), rendered in first-seen order, one `<section>` per chapter, one
//! `<article class="doc-entry">` per name with a runnable usage example.

use std::collections::BTreeSet;
use std::fmt::Write as _;

/// One corpus row: `(name, chapter, doc, example)`. Aliased so the grouping
/// structures below stay under clippy's `type_complexity` threshold.
type Entry<'a> = (&'a str, &'a str, &'a str, &'a str);

fn main() {
    let corpus = nodejs::lsp::corpus();
    ...
    .replace("__NODEJS_VERSION__", env!("CARGO_PKG_VERSION"));
...
fn build_body(corpus: &[Entry]) -> String {
    ...
    for (chapter, entries) in &chapters {
        ...
        // name (e.g. `slice`) repeats across chapters.
...
/// e.g. `Keyword` -> `keyword`, `Array method` -> `array-method`. Used for the
/// `id="ch-…"` chapter anchors.
...
<head>
  ...
  <meta name="description" content="node-js — Builtin reference. Keywords, globals, and core object/type methods available in the current node-js build. MIT licensed.">
  <title>node-js &mdash; Builtin Reference</title>
  ...
  <div class="app tutorial-app" id="docsApp">
    ...
    <header class="tutorial-header">
      <div class="tutorial-header-inner">
        <div>
          <h1 class="tutorial-brand">// NODE-JS — BUILTIN REFERENCE</h1>
          <nav class="tutorial-crumbs" aria-label="Breadcrumb">
            <a href="index.html">Docs</a>
            <span class="sep">/</span>
            <span class="current">Builtin Reference</span>
            <span class="sep">/</span>
            <a href="https://github.com/MenkeTechnologies/node-js" target="_blank" rel="noopener noreferrer">GitHub</a>
          </nav>
          <p class="docs-build-line">node-js v__NODEJS_VERSION__ · JavaScript on fusevm · lex/parse → AST → bytecode → Cranelift JIT · JsHost object heap · binary <code>node</code> · MIT · in active development</p>
        </div>
        <div class="tutorial-toolbar">
          ...
          <a class="btn btn-secondary" href="https://github.com/MenkeTechnologies/node-js" target="_blank" rel="noopener noreferrer">GitHub</a>
        </div>
      </div>
    </header>
    ...
    <main class="tutorial-main">
      ...
      <p class="tutorial-subtitle">Every reserved keyword, global, and core object/type method the current node-js build recognizes, grouped by keyword set then globals then namespace then type. This page is generated from the corpus (<code>src/lsp.rs</code>) by the <code>gen-docs</code> binary, so it stays in sync with what the runtime actually knows about. Keywords mirror <code>parser.rs</code>; each global and method mirrors a real dispatch arm in <code>src/builtins.rs</code>.</p>
"#;
