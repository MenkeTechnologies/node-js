...
pub fn emit_executable(prog: &Program, out: &Path) -> Result<(), String> {
    ...
    if cfg!(target_os = "macos") {
        cmd.args([
            ...
            // (stamp the Mach-O platform at object emission); a node-js-side
...
/// Locate `libnodejs.a` (a sibling of the running `node` binary, or
/// `$NODE_JS_STATICLIB`).
...
/// The AOT runtime hook: install the node-js builtins + numeric hook and reload
/// the embedded function/try tables before the main chunk runs. Required link
/// symbol for a standalone node-js AOT binary.
///
/// # Safety
