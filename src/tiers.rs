...
pub struct ChunkTiers {
    /// Which chunk this is — `main` for a whole JavaScript program.
    pub name: String,
...
/// has gone round enough times to be recorded. node-js writes program output
...
///
/// A JavaScript program is not one chunk: the top level, every function and
/// arrow body, and every arm of a `try` lower to their own chunk. A hot loop
